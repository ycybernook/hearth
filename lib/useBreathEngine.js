 ...
 */
export function useBreathEngine(refs, { sound, haptics }) {
  ...
  const opts = useRef({ sound, haptics });
  const reduced = useRef(false);

  useEffect(() => {
    opts.current = { sound, haptics };
  }, [sound, haptics]);
  ...
  const start = useCallback(
    (pattern, totalSeconds) => {
      ...
      const frame = (now) => {
        ...
        if (t >= totalSeconds) {
          if (opts.current.sound) playChime();
    ...
    [paint, refs, releaseWakeLock, requestWakeLock]
  );
